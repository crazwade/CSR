  ...
  >
    ...
    <div v-if="!switchValue">
      ...
      <span class="flex justify-center items-center">
        ...
        <el-icon
          ...
          v-if="lastWeekVisable"
          @click="lastWeekVisable = !lastWeekVisable"
          size="25px"
          ><View
        /></el-icon>
        <el-icon
      ...
      <div v-if="lastWeekVisable">
        ...
        >
          <el-select
            v-model="lessonDetail.lastLessonKey"
            class="m-2"
            placeholder="Select"
            @change="lessonDetail.lastLessonTitle = null"
          >
            <el-option
        ...
        >
          <el-select
            v-model="lessonDetail.lastLessonTitle"
            class="m-2"
            placeholder="Select"
          >
            <el-option
              v-for="(item, index) in lessonContent[
                lessonDetail.lastLessonKey as keyof typeof lessonContent
              ] || []"
              :key="index"
        ...
        >
          <el-select
            v-model="lessonDetail.lastLessonProcess1"
            class="m-2 w-[60px]"
            placeholder="Select"
          >
            <el-option
          ...
          <div>-</div>
          <el-select
            v-model="lessonDetail.lastLessonProcess2"
            class="m-2 w-[60px]"
            placeholder="Select"
          >
            <el-option
    ...
    <div>
      <span class="flex justify-center items-center">
        ...
        <el-icon
          class="mx-3 cursor-pointer"
          v-if="thisWeekVisable"
          @click="thisWeekVisable = !thisWeekVisable"
          size="25px"
          ><View
        /></el-icon>
        <el-icon
      ...
      <div v-if="thisWeekVisable">
        ...
        >
          <el-select
            v-model="lessonDetail.lessonKey"
            class="m-2"
            placeholder="Select"
            @change="lessonDetail.lessonTitle = null"
          >
            <el-option
              v-for="(item, index) in lessonKey"
              :key="index"
        ...
        >
          <el-select
            v-model="lessonDetail.lessonTitle"
            class="m-2"
            placeholder="Select"
          >
            <el-option
              v-for="(item, index) in lessonContent[
                lessonDetail.lessonKey as keyof typeof lessonContent
              ] || []"
              :key="index"
              :label="`${item.key} - ${item.lessonName}`"
              :value="item.key"
        ...
        >
          <el-select
            v-model="lessonDetail.lessonProcess1"
            class="m-2 w-[60px]"
            placeholder="Select"
          >
            <el-option
          ...
          <div>-</div>
          <el-select
            v-model="lessonDetail.lessonProcess2"
            class="m-2 w-[60px]"
            placeholder="Select"
          >
            <el-option
    ...
    <div>
      <span class="flex flex-col justify-center items-center">
        <div class="my-3 font-semibold text-base mx-3">
          ==課堂表現自動生成==
        </div>
        <div class="text-red-500 font-extrabold">特殊狀況建議自行填寫</div>
      </span>
      <div>
        <el-form-item label="表現：" class="flex justify-center items-center">
          <el-select
            v-model="lessonDetail.behavior"
            class="m-2"
            placeholder="Select"
            @change="classBehaviorSelect"
          >
...
const generatedTextContent = computed(() => {
  ...
  const thisWeekExtra: string =
    props.extracurriculars === ""
      ? "\n\n<<尚未添加動腦時間>>"
      : `\n\n${props.extracurriculars}`;

  if (
    lessonDetail.lastLessonKey === null &&
  ...
  const getLastLessonTitle = lessonContent[
    lessonDetail.lastLessonKey as keyof typeof lessonContent
  ].find((item) => item.key === lessonDetail.lastLessonTitle);
